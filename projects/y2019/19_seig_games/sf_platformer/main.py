...
class Game:
    ...
    def load_data(self):
        ...
        for i in range(1, 4):
            self.cloud_images.append(
                pg.image.load(
                    path.join(img_dir, "cloud{}.png".format(i))
                ).convert()
            )
    ...
    def update(self):
        ...
        if now - self.mob_timer > MOB_FREQ + random.choice(
            [-1000, -500, 0, 500, 1000]
        ):
            self.mob_timer = now
            Mob(self)
    ...
    def show_start_screen(self):
        ...
        self.draw_text(
            "Press a key to play", 22, WHITE, WIDTH / 2, HEIGHT * 3 / 4
        )
        self.draw_text(
            "HIGH SCORE: {}".format(str(self.highscore)),
            22,
            WHITE,
            WIDTH / 2,
            15,
    ...
    def show_go_screen(self):
        ...
        self.draw_text(
            "Press a key to play again", 22, WHITE, WIDTH / 2, HEIGHT * 3 / 4
        )
        if self.score > self.highscore:
            self.highscore = self.score
            self.draw_text(
                "NEW HIGH SCORE!", 22, WHITE, WIDTH / 2, HEIGHT / 2 + 40
            )
